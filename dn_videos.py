...
class BiliBili:
    ...
    def get_download_url(self, arcurl):
        ...
        durl = html['durl']
        download_url = [] # 根据视频长度被分了好几段
        for i in range(len(durl)):
            download_url.append(durl[i]['url'])
        url = durl[0]['url']
        if 'mirrork' in url: # 从视频网址里找到弹幕id
            oid = url.split('/')[6]
        else:
            id_ = url.split('/')[7]
            if len(id_) >= 10:
    ...
    def get_danmu(self, oid, filename):
        ...
        time.sleep(0.5)
        three.Danmaku2ASS(danmu_name, danmu_ass, 1280, 720)
    ...
    def search_videos(self, keyword, pages):
        ...
        Returns:
            无
        """
        if self.dir not in os.listdir(self.dir): # 是否下载过
            os.mkdir(self.dir)
        for page in range(1, pages + 1): # 遍历搜索的每一页
            search_url = 'https://api.bilibili.com/x/web-interface/search/type?jsonp=jsonp&search_type=video&keyword={}&page={}'.format(
                keyword, page) # 搜索第二页能找到
            titles, arcurls = self.search_video(search_url) # 请求每一搜索页
            for index, arcurl in enumerate(arcurls): # 遍历arcurls和titles
                title = titles[index]
                for c in u'´☆❤◦\/:*?"<>|': # 去除标题中的特殊符号
                    title = title.replace(c, '')
                if title + '.flv' not in os.listdir(self.dir): # 如果没有下载过
                    download_url, oid = self.get_download_url(arcurl) # 获取视频和弹幕下载地址
                    movies = []
                    for i in range(len(download_url)): # 遍历单个视频下载列表
                        if download_url[i] != '' and oid != '':
                            fname = title + '_' + str(i + 1) + '.flv'
                            movies.append(fname)
                            print('第[ %d ]页:视频[ %s ]下载中:' % (page, fname))
                            self.video_downloader(download_url[i], fname) # 下载视频
                            print('视频下载完成!')
                    if len(movies) > 1: # 多个视频创建个文本保存下视频文件名列表, 为了合并
...
if __name__ == '__main__':
    ...
    parser.add_argument('-p', '--pages', required=True, help=_('the number of pages for downloading'), type=int,
                        default=1)
